...
//cout<<"cycle "<<c_num <<" " <<ins<<endl;
 ...
 //cout<<c_num<<"  "<<(int)Tres[17] <<" "<<endl;
  ...
  //cout<<rss<<" "<<rts<<" "<<rees;
	if ((rss==0&&rts==0&&rees==1)||(rss==1&&rts==1&&rees==0))errorfile << "In cycle " << c_num << ": Number Overflow" << endl; Tres[rd] = (unsigned int)((int)Tres[rs] - (int)Tres[rt]); } }
	...
	/*20lb*/if (opcode == "100000") { int rs = BtoD(ins.substr(6, 5)); int rt = BtoD(ins.substr(11, 5)); int imm = BtoDs(ins.substr(16)); int addrt =(int) Tres[rs] +(int) imm;long addr=addrt+8; if (rt == 0) errorfile << "In cycle " << c_num << ": Write $0 Error" << endl;  { long re = (long)Tres[rs] + (long)imm; if (re>2147483647 || re<intmin)errorfile << "In cycle " << c_num << ": Number Overflow" << endl; if ((unsigned long)addrt > 1023||addrt <0) { errorfile << "In cycle " << c_num << ": Address Overflow" << endl; go_halt = true; } else if(!go_halt){ Tres[rt] = HtoD(addr, 1, false); if (Tres[rt] >= 128) Tres[rt] = Tres[rt] | 0xFFFFFF00; } } }
	/*21lh*/if (opcode == "100001") { int rs = BtoD(ins.substr(6, 5)); int rt = BtoD(ins.substr(11, 5)); int imm = BtoDs(ins.substr(16)); int addrt =(int) Tres[rs] +(int) imm;long addr=addrt+8; if (rt == 0) errorfile << "In cycle " << c_num << ": Write $0 Error" << endl;  { long re = (long)(int)Tres[rs] + (long)(int)imm; if (re>2147483647 || re<intmin)errorfile << "In cycle " << c_num << ": Number Overflow" << endl;if ((unsigned long)(addrt + 1) > 1023||addrt <0) { errorfile << "In cycle " << c_num << ": Address Overflow" << endl; go_halt = true; }  if (addrt % 2 != 0) { errorfile << "In cycle " << c_num << ": Misalignment Error" << endl; go_halt = true; } else if(!go_halt){ Tres[rt] = HtoD(addr, 2, false); if (Tres[rt] > 32767) Tres[rt] = Tres[rt] | 0xFFFF0000; } } }
	/*23lw*/if (opcode == "100011") { int rs = BtoD(ins.substr(6, 5)); int rt = BtoD(ins.substr(11, 5)); int imm = BtoDs(ins.substr(16)); int addrt =(int) Tres[rs] +(int) imm;long addr=addrt+8; if (rt == 0) errorfile << "In cycle " << c_num << ": Write $0 Error" << endl;  { long re = (long)(int)Tres[rs] + (long)(int)imm; if (re>2147483647 || re<intmin)errorfile << "In cycle " << c_num << ": Number Overflow" << endl; if ((unsigned long)(addrt + 3) > 1023||addrt <0) { errorfile << "In cycle " << c_num << ": Address Overflow" << endl; go_halt = true; }  if (addrt % 4 != 0) { errorfile << "In cycle " << c_num << ": Misalignment Error" << endl; go_halt = true; } else if(!go_halt)Tres[rt] = HtoD(addr, 4, false); } }
	/*24lbu*/if (opcode == "100100") { int rs = BtoD(ins.substr(6, 5)); int rt = BtoD(ins.substr(11, 5)); int imm = BtoDs(ins.substr(16)); int addrt =(int) Tres[rs] +(int) imm;long addr=addrt+8; if (rt == 0) errorfile << "In cycle " << c_num << ": Write $0 Error" << endl;  { long re = (long)(int)Tres[rs] + (long)(int)imm; if (re>2147483647 || re<intmin)errorfile << "In cycle " << c_num << ": Number Overflow" << endl; if ((unsigned long)addrt > 1023||addrt <0) { errorfile << "In cycle " << c_num << ": Address Overflow" << endl; go_halt = true; } else if(!go_halt)Tres[rt] = HtoD(addr, 1, false); } }
	/*25lhu*/if (opcode == "100101") { int rs = BtoD(ins.substr(6, 5)); int rt = BtoD(ins.substr(11, 5)); int imm = BtoDs(ins.substr(16)); int addrt =(int) Tres[rs] +(int) imm;long addr=addrt+8; if (rt == 0) errorfile << "In cycle " << c_num << ": Write $0 Error" << endl;  { long re = (long)(int)Tres[rs] + (long)(int)imm; if (re>2147483647 || re<intmin)errorfile << "In cycle " << c_num << ": Number Overflow" << endl; if ((unsigned long)(addrt + 1) > 1023||addrt <0) { errorfile << "In cycle " << c_num << ": Address Overflow" << endl; go_halt = true; }  if (addrt % 2 != 0) { errorfile << "In cycle " << c_num << ": Misalignment Error" << endl; go_halt = true; } else if(!go_halt)Tres[rt] = HtoD(addr, 2, false); } }
	/*28sb*/if (opcode == "101000") { int rs = BtoD(ins.substr(6, 5)); int rt = BtoD(ins.substr(11, 5)); int imm = BtoDs(ins.substr(16)); int addrt =(int) Tres[rs] +(int) imm;long addr=addrt+8; {long re = (long)(int)Tres[rs] + (long)(int)imm; if (re>2147483647 || re<intmin)errorfile << "In cycle " << c_num << ": Number Overflow" << endl; if ((unsigned long)addrt > 1023||addrt <0) { errorfile << "In cycle " << c_num << ": Address Overflow" << endl; go_halt = true; } else if(!go_halt){ save(addr, 1,( Tres[rt] & 0x000000FF)); // cout<<addr<<" "<<(int)buffer[addr]<<" "<<(int)buffer[addr+1]<<" "<<(int)buffer[addr+2]<<" "<<(int)buffer[addr+3];
  }}
  }
	/*29sh*/if (opcode == "101001") { int rs = BtoD(ins.substr(6, 5)); int rt = BtoD(ins.substr(11, 5)); int imm = BtoDs(ins.substr(16)); int addrt =(int) Tres[rs] +(int) imm;long addr=addrt+8; {long re = (long)(int)Tres[rs] + (long)(int)imm; if (re>2147483647 || re<intmin)errorfile << "In cycle " << c_num << ": Number Overflow" << endl; if ((unsigned long)(addrt + 1) > 1023||addrt <0) { errorfile << "In cycle " << c_num << ": Address Overflow" << endl; go_halt = true; }  if (addrt % 2 != 0) { errorfile << "In cycle " << c_num << ": Misalignment Error" << endl; go_halt = true; } else if(!go_halt){ save(addr, 2,( Tres[rt] & 0x0000FFFF));
  //cout<<addr<<" "<<(int)buffer[addr]<<" "<<(int)buffer[addr+1]<<" "<<(int)buffer[addr+2]<<" "<<(int)buffer[addr+3];
   }}}
	/*2Bswv */if (opcode == "101011") { int rs = BtoD(ins.substr(6, 5)); int rt = BtoD(ins.substr(11, 5)); int imm = BtoDs(ins.substr(16)); int addrt =(int) Tres[rs] +(int) imm;long addr=addrt+8; {long re = (long)(int)Tres[rs] + (long)(int)imm; if (re>2147483647 || re<intmin)errorfile << "In cycle " << c_num << ": Number Overflow" << endl; if ((unsigned long)(addrt + 3) > 1023||addrt <0) { errorfile << "In cycle " << c_num << ": Address Overflow" << endl; go_halt = true; }  if (addrt % 4 != 0) { errorfile << "In cycle " << c_num << ": Misalignment Error" << endl; go_halt = true; } else  if(!go_halt){save(addr, 4, Tres[rt]);
  //cout<<addr<<" "<<(int)buffer[addr]<<" "<<(int)buffer[addr+1]<<" "<<(int)buffer[addr+2]<<" "<<(int)buffer[addr+3]; 
  }}
   }
