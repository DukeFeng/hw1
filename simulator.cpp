...
void deal_ins(unsigned long num,int c_num) {
	...
	if (opcode == "000000") {
		...
		/*18*/if (func == "011000") { if (multy_deal)cout << "In cycle " << c_num << ": Overwrite HI-LO registers" << endl;  { multy_deal = true; long rs = BtoD(ins.substr(6, 5)); long rt = BtoD(ins.substr(11, 5));  unsigned long long re = (long)Tres[rs] * (long)Tres[rt];if (re>LLONG_MAX||re<LLONG_MIN)cout << "In cycle " << c_num << ": Number Overflow" << endl; THi = re / 0x100000000; TLo = re % 0x100000000; } }
		...
		/*20*/if (func == "100000") { long rs = BtoD(ins.substr(6, 5)); long rt = BtoD(ins.substr(11, 5)); long rd = BtoD(ins.substr(16, 5)); if (rd == 0) cout << "In cycle " << c_num << ": Write $0 Error" << endl; else { unsigned long re = (long)Tres[rs] + (long)Tres[rt]; if (re>LONG_MAX||re<LONG_MIN)cout << "In cycle " << c_num << ": Number Overflow" << endl; Tres[rd] = (unsigned long)re; } }
		...
		/*22*/if (func == "100010") { long rs = BtoD(ins.substr(6, 5)); long rt = BtoD(ins.substr(11, 5)); long rd = BtoD(ins.substr(16, 5)); if (rd == 0) cout << "In cycle " << c_num << ": Write $0 Error" << endl; else { long long re = Tres[rs] - Tres[rt]; if (re>LONG_MAX || re<LONG_MIN)cout << "In cycle " << c_num << ": Number Overflow" << endl; Tres[rd] = (unsigned long)re; } }
	...
	/*08*/if (opcode == "001000") { long rs = BtoD(ins.substr(6, 5)); long rt = BtoD(ins.substr(11, 5)); long imm = BtoDs(ins.substr(16));  if (rt == 0) cout << "In cycle " << c_num << ": Write $0 Error" << endl; else {unsigned long re = ((long)Tres[rs] + (long)imm); if (re>LONG_MAX || re<LONG_MIN)cout << "In cycle " << c_num << ": Number Overflow" << endl; Tres[rt] = re; } }
	...
	/*20*/if (opcode == "100000") { long rs = BtoD(ins.substr(6, 5)); long rt = BtoD(ins.substr(11, 5)); long imm = BtoDs(ins.substr(16)); long addr = Tres[rs] + imm + 8; if (rt == 0) cout << "In cycle " << c_num << ": Write $0 Error" << endl; else { unsigned long re = (long)Tres[rs] + (long)imm; if (re>LONG_MAX || re<LONG_MIN)cout << "In cycle " << c_num << ": Number Overflow" << endl; if (addr - 8 > 1023) { cout << "In cycle " << c_num << ": Address Overflow" << endl; go_halt = true; } else { Tres[rt] = HtoD(addr, 1, false); if (Tres[rt] >= 128) Tres[rt] = Tres[rt] | 0xFFFFFF00; } } }
	/*21*/if (opcode == "100001") { long rs = BtoD(ins.substr(6, 5)); long rt = BtoD(ins.substr(11, 5)); long imm = BtoDs(ins.substr(16)); long addr = Tres[rs] + imm + 8; if (rt == 0) cout << "In cycle " << c_num << ": Write $0 Error" << endl; else { unsigned long re = (long)Tres[rs] + (long)imm; if (re>LONG_MAX || re<LONG_MIN)cout << "In cycle " << c_num << ": Number Overflow" << endl; if (addr - 8 + 1 > 1023) { cout << "In cycle " << c_num << ": Address Overflow" << endl; go_halt = true; } else if ((addr - 8) % 2 != 0) { cout << "In cycle " << c_num << ": Data misaligned" << endl; go_halt = true; } else { Tres[rt] = HtoD(addr, 2, false); if (Tres[rt] > 32767) Tres[rt] = Tres[rt] | 0xFFFF0000; } } }
	/*23*/if (opcode == "100011") { long rs = BtoD(ins.substr(6, 5)); long rt = BtoD(ins.substr(11, 5)); long imm = BtoDs(ins.substr(16)); long addr = Tres[rs] + imm + 8; if (rt == 0) cout << "In cycle " << c_num << ": Write $0 Error" << endl; else { unsigned long re = (long)Tres[rs] + (long)imm; if (re>LONG_MAX || re<LONG_MIN)cout << "In cycle " << c_num << ": Number Overflow" << endl; if (addr - 8 + 3 > 1023) { cout << "In cycle " << c_num << ": Address Overflow" << endl; go_halt = true; } else if ((addr - 8) % 4 != 0) { cout << "In cycle " << c_num << ": Data misaligned" << endl; go_halt = true; } else Tres[rt] = HtoD(addr, 4, false); } }
	...
	/*28*/if (opcode == "101000") { long rs = BtoD(ins.substr(6, 5)); long rt = BtoD(ins.substr(11, 5)); long imm = BtoD(ins.substr(16)); long addr = Tres[rs] + imm + 8; {unsigned long re = (long)Tres[rs] + (long)imm; if (re>LONG_MAX || re<LONG_MIN)cout << "In cycle " << c_num << ": Number Overflow" << endl; if (addr - 8 > 1023) { cout << "In cycle " << c_num << ": Address Overflow" << endl; go_halt = true; } else save(addr, 1, Tres[rt] & 0x000000FF); } }
	/*29*/if (opcode == "101001") { long rs = BtoD(ins.substr(6, 5)); long rt = BtoD(ins.substr(11, 5)); long imm = BtoD(ins.substr(16)); long addr = Tres[rs] + imm + 8; {unsigned long re = (long)Tres[rs] + (long)imm; if (re>LONG_MAX || re<LONG_MIN)cout << "In cycle " << c_num << ": Number Overflow" << endl; if (addr - 8 + 1 > 1023) { cout << "In cycle " << c_num << ": Address Overflow" << endl; go_halt = true; } else if ((addr - 8) % 2 != 0) { cout << "In cycle " << c_num << ": Data misaligned" << endl; go_halt = true; } else save(addr, 2, Tres[rt] & 0x0000FFFF); } }
	/*2B*/if (opcode == "101011") { long rs = BtoD(ins.substr(6, 5)); long rt = BtoD(ins.substr(11, 5)); long imm = BtoD(ins.substr(16)); long addr = Tres[rs] + imm + 8; {unsigned long re = (long)Tres[rs] + (long)imm; if (re>LONG_MAX || re<LONG_MIN)cout << "In cycle " << c_num << ": Number Overflow" << endl; if (addr - 8 + 3 > 1023) { cout << "In cycle " << c_num << ": Address Overflow" << endl; go_halt = true; } else if ((addr - 8) % 4 != 0) { cout << "In cycle " << c_num << ": Data misaligned" << endl; go_halt = true; } else  save(addr, 4, Tres[rt]); } }
